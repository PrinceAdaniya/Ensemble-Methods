...
class DecisionTree:
    ...
    def _best_split(self, X, y):
        ...
        if max_v - min_v == 0:
            ...
            # -------- DATASET-SPECIFIC NORMALIZED RANGE --------
            if CURRENT_DATASET == "breast":
                low, high = 0.00, 0.90
            elif CURRENT_DATASET == "sonar":
                low, high = 0.25, 0.95
            else:
                low, high = 0.2, 0.5  # fallback

            rand_norm = np.random.uniform(low, high)
            thresh = min_v + rand_norm * (max_v - min_v)
            # --------------------------------------------------
...
def run_experiment(dataset_name):
    ...
    plt.legend()
    plt.savefig(f"{GRAPH_DIR}/test2_{dataset_name}_AE.png")
    plt.close()
    ...
    plt.legend()
    plt.savefig(f"{GRAPH_DIR}/test2_{dataset_name}_SC.png")
    plt.close()

    end_time = time.time()

    print(f"✅ Graphs saved for {dataset_name}")
    print(f"⏱️ Time taken for {dataset_name}: {end_time - start_time:.2f} seconds")

    final_acc = np.mean(acc_results)
    best_acc = np.max(acc_results)

    print(f"🎯 Final Average Accuracy for {dataset_name}: {final_acc:.4f}")
